...
checkReplicaSet() {
#   echo 'rs.status()' | mongosh --quiet --host $HOST --port $PORT --username $USER --password $PASSWORD --authenticationDatabase $AUTH_DB | grep -q 'setName'
#   docker exec -it mongodb sh -c "echo 'rs.status()' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB | grep -q 'setName'"
  echo 'rs.status()' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB | grep -q 'setName'
}
...
#   docker exec -it mongodb sh -c "echo 'rs.initiate()' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB"
  echo 'rs.initiate()' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB
...
#   docker exec -it mongodb sh -c "echo 'var cfg = rs.config(); cfg.members[0].host = \"localhost:27017\"; rs.reconfig(cfg);' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB"
  echo 'var cfg = rs.config(); cfg.members[0].host = "localhost"; rs.reconfig(cfg);' | mongo --quiet -u $USER -p $PASSWORD --authenticationDatabase $AUTH_DB
fi
